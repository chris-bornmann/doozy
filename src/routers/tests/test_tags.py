...
def test_create_tag_returns_tag(admin_client):
...
def test_create_tag_too_long(admin_client):
    client, _ = admin_client
    assert client.post("/tags/", json={"name": "a" * 17}).status_code == 422


def test_create_tag_empty_name(admin_client):
    client, _ = admin_client
    assert client.post("/tags/", json={"name": ""}).status_code == 422


def test_create_tag_duplicate(admin_client):
    client, _ = admin_client
...
def test_list_tags_alphabetical(admin_client):
    client, _ = admin_client
...
def test_list_tags_default_page_size(admin_client):
    client, _ = admin_client
    for i in range(15):
...
def test_list_tags_custom_size(admin_client):
    client, _ = admin_client
    for i in range(5):
...
def test_list_tags_match(admin_client):
    client, _ = admin_client
...
def test_list_tags_match_no_results(admin_client):
    client, _ = admin_client
...
def test_delete_tag_removes_item_tag_entries(admin_client, session):
